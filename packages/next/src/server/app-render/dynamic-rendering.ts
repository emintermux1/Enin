...
export function throwIfDisallowedEmptyShell(
  ...
  if (dynamicValidation.hasDynamicMetadata) {
    throw new InvariantError(
      'Expected `generateMetadata` to not block a prerender but it did.'
    )
  }
  ...
  if (serverDynamic.syncDynamicErrorWithStack) {
    ...
    // could be completed.
    if (!serverDynamic.syncDynamicLogged) {
      // During validation in dev we already logged this error but during
      // builds we need to ensure the actual error is output
  ...
  if (clientDynamic.syncDynamicErrorWithStack) {
    ...
    if (!clientDynamic.syncDynamicLogged) {
  ...
  if (dynamicValidation.hasDynamicViewport) {
    throw new StaticGenBailoutError(
      `Route "${route}" has a \`generateViewport\` that depends on Request data (\`cookies()\`, etc...) or uncached external data (\`fetch(...)\`, etc...) without explicitly allowing fully dynamic rendering. See more info here: https://nextjs.org/docs/messages/next-prerender-dynamic-viewport.mdx`
    )
  }
}
