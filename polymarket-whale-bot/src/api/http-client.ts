...
export class HttpClient {
  ...
  constructor(
    baseURL: string,
    private readonly options: { timeoutMs: number; retries: number; minSpacingMs: number },
  ) {
    ...
    return this.schedule(async () => {
      ...
      for (let attempt = 0; attempt <= this.options.retries; attempt += 1) {
        try {
          ...
          logger.warn(`HTTP retry ${attempt + 1}/${this.options.retries} for ${url}`, status ?? axiosError.message);
          await delay(backoff);
        }
      }
      throw lastError;
  ...
  private async schedule<T>(task: () => Promise<T>): Promise<T> {
    const scheduled = this.queue.then(async () => {
      const waitMs = Math.max(0, this.options.minSpacingMs - (Date.now() - this.lastRequestAt));
      if (waitMs > 0) {
        await delay(waitMs);
      }
